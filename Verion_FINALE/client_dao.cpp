...
bool client_dao::create(const Client& c)
{
    ...
    if (ok) {
        ...
        QMessageBox::critical(nullptr, "❌ Error",
                              QString("Error during insertion: %1")
...
bool client_dao::update(const Client& c)
{
    ...
    if (!exists(c.CIN)) {
        QMessageBox::warning(nullptr, "Client Not Found",
                             QString("No client with CIN '%1' exists.")
                                 .arg(c.CIN));
        return false;
    }
...
{
    const QString trimmed = cin.trimmed();

    if (trimmed.isEmpty()) {
        QMessageBox::warning(nullptr, "Empty CIN",
                             "Please enter a CIN to delete.");
        return false;
    }

    if (!exists(trimmed)) {
        QMessageBox::warning(nullptr, "❌ Client Not Found",
                             QString("No client with CIN '%1' was found in the database.")
                                 .arg(trimmed));
        return false;
    }
    ...
    if (ok) {
        db.commit();
        QMessageBox::information(nullptr, "✅ Success",
                                 QString("Client with CIN '%1' has been deleted successfully.")
                                     .arg(trimmed));
    } else {
        db.rollback();
        QMessageBox::critical(nullptr, "❌ Error",
                              QString("Error during deletion: %1")
                                  .arg(q.lastError().text()));
    }
    return ok;
}
...
bool client_dao::validateClient(const Client& c, QString& errorMessage)
{
    ...
    if (!cinRx.match(c.CIN.trimmed()).hasMatch()) {
        errorMessage = "❌ CIN must contain exactly 8 digits.";
        return false;
    }
...
void client_dao::journalLoad(QListWidget *list)
{
    ...
    // afficher les plus récentes en premier, max 50
...
// ============================
//   getByCin : pour comparer
// ============================

bool client_dao::getByCin(const QString &cin, Client &out)
{
    QSqlQuery q;
    q.prepare(R"(
        SELECT CIN, FIRST_NAME, LAST_NAME, EMAIL, PHONE_NBR, ADRESS, AGE
    ...
    out.FIRST_NAME = q.value(1).toString();
    out.LAST_NAME  = q.value(2).toString();
    out.EMAIL      = q.value(3).toString();
    out.PHONE_NBR  = q.value(4).toString();
    out.ADRESS     = q.value(5).toString();
    out.AGE        = q.value(6).toInt();
