...
bool Commande::supprimer(QString code)
{
    ...
    // SUPPRESSION DIRECTE de TAB_COMMANDE seulement
    ...
    if (success) {
        qDebug() << "✅ Suppression TAB_COMMANDE - Lignes affectées:" << query.numRowsAffected();
    } else {
        qDebug() << "❌ Erreur suppression TAB_COMMANDE :" << query.lastError().text();
    }

    // Ne pas vérifier CONCERNER du tout
    return success;
}
