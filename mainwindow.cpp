    ...
    : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    lcdPort->setPortName("COM5");     // ⚠ Mets ton port LCD ici
    lcdPort->setBaudRate(QSerialPort::Baud9600);

    if (!lcdPort->open(QIODevice::WriteOnly)) {
        QMessageBox::warning(this, "LCD Error",
                             "Unable to connect to LCD Arduino on COM5.");
    }
    arduino = new ArduinoManager(this);

        if (!arduino->connectArduino("COM6")) {   // adapte COM3 si besoin
            QMessageBox::critical(this, "Arduino Error",
                                  "Unable to connect to Arduino on COM6.");
        }

        connect(arduino, &::ArduinoManager::ideReceived,
                this, &MainWindow::onideReceived);

        connect(arduino, &ArduinoManager::errorOccurred,
                this, [](const QString &msg) {
                    QMessageBox::critical(nullptr, "Arduino Error", msg);
                });
...
void MainWindow::sendOrderToLCD(const QString &total, const QString &products)
{
    ...
    // 👉 ENVOYER EN ASCII pur
    QByteArray ascii = message.toLatin1();   // IMPORTANT !!!

    lcdPort->write(ascii);
}
void MainWindow::onideReceived(const QString &ide)
{
    // 1) Vérifier que l'IDE reçu est bien un nombre
    bool ok = false;
    int ideInt = ide.toInt(&ok);
    if (!ok) {
        QMessageBox::warning(this,
                             "Invalid IDE",
                             "Invalid IDE received from Arduino: " + ide);
        if (arduino)
            arduino->sendToArduino("DENIED");
        return;
    }

    // 2) Requête : vérifier si l'employé existe
    QSqlQuery q;
    q.prepare("SELECT FULLNAME FROM EMPLOYEES WHERE IDE = :id");
    q.bindValue(":id", ideInt);
    ...
    if (q.next()) {
        ...
        QMessageBox::information(this,
                                 "Access Granted",
                                 "Employee exists: " + fullname +
                                     "\nDoor opening...");

        // 3) Arduino ouvre la porte
        if (arduino)
            arduino->sendToArduino("OK");

        // 4) Sauvegarder DATE_ENTREE (VARCHAR2)
        QString now = QDateTime::currentDateTime().toString("dd/MM/yyyy HH:mm");

        QSqlQuery updateQ;
        updateQ.prepare(
            "UPDATE EMPLOYEES "
            "SET DATE_ENTREE = :date "
            "WHERE IDE = :id"
            );
        updateQ.bindValue(":date", now);
        updateQ.bindValue(":id", ideInt);

        if (!updateQ.exec()) {
            QMessageBox::warning(this,
                                 "Database Error",
                                 "Failed updating DATE_ENTREE:\n" + updateQ.lastError().text());
        } else {
            QMessageBox::information(this,
                                     "Entry Logged",
                                     "Employee " + fullname +
...
{
    // 1) Lancer Python pour générer le CSV
    ...
    // 📌 Chemin dynamique : dossier de l'exe
    QString appDir = QCoreApplication::applicationDirPath();

    // 📌 CSV généré dans :  <exe>/ai/articles_classified.csv
    QString csvPath = appDir + "/ai/articles_classified.csv";
    ...
    while (!in.atEnd()) {
        ...
        if (si.clusterName.startsWith("Dormant", Qt::CaseInsensitive))
            si.recommendation = "Proposer une promo, mise en avant ou liquidation.";
        else if (si.clusterName.startsWith("Intermédiaire", Qt::CaseInsensitive))
            si.recommendation = "Surveiller stock et ventes, ajustement progressif.";
        else if (si.clusterName.startsWith("Actif", Qt::CaseInsensitive))
            si.recommendation = "Très bonne rotation : surveiller le stock pour éviter rupture.";
        else
...
// Add edit + delete action buttons for an article row
void MainWindow::addActionButtonsForRow(int row)
{
    ...
    const char* kDelPath = ":/image/ressource/delete.png";
    if (QFile::exists(kDelPath)) delBtn->setIcon(QIcon(kDelPath));
    else delBtn->setText("DEL");
    delBtn->setIconSize(QSize(18,18));
    delBtn->setStyleSheet("QPushButton{border:none;color:#b00;}");
    delBtn->setFixedSize(36,26);
    ...
    connect(delBtn, &QPushButton::clicked, this, [this, delBtn](){
        QWidget *cell = delBtn->parentWidget();
        QPoint p = cell->mapTo(ui->tab_Art->viewport(), QPoint(2,2));
        int row = ui->tab_Art->indexAt(p).row();
        if (row < 0) return;
        ...
        QTableWidgetItem* skuItem = ui->tab_Art->item(row, 1);
        if (!skuItem) return;
        bool ok=false; const int sku = skuItem->text().toInt(&ok);
        if (!ok) return;
...
void MainWindow::rafraichirTableau()
{
    ...
    QStringList headers = {"CODE", "DATE_COMMANDE", "PRODUCTS", "TOTAL", "STATUS", "CLIENT"};
...
void MainWindow::on_pushButton_ajouter_clicked()
{
    ...
    else
    {
        ...
        if (success)
        {
            ...
            QString totalStr = ui->lineEdit_total->text().trimmed();
            QString productsStr = ui->lineEdit_products->text().trimmed();

            sendOrderToLCD(totalStr, productsStr);
    ...
    // 🔧 CORRECTION IMPORTANTE : Forcer le rafraîchissement complet
    rafraichirTableau();
}
