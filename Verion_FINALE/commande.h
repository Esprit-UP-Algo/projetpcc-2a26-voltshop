...
public:
    ...
    bool modifier();
    bool updatePstatus(QString code, QString pstatus);  // NEW: Update product status only
...
public:
    // Getters for pstatus
    QString getPstatus() const { return pstatus; }
    void setPstatus(const QString &status) { pstatus = status; }

private:
    QString code;
    QString client, produits, statut;
    QString pstatus = "";  // Product status from Arduino
    QDate date_commande;
    double total;
